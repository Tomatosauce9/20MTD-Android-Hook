...
{
    void *handle = xdl_open(lib, 0);
    if(handle) {
        void *fn = xdl_sym(handle, name, nullptr);
        IL2CPP_LOGI("lib:%s, name:%s", lib, name);
        if (fn) {
            IL2CPP_LOGI("addr:%lx", fn);
            return fn;
        }
    }
    not_found_export++;
...
int Il2Cpp::Attach(const char *libname) {
    ...
    il2cpp_runtime_invoke = (void *(*)(void *, void *, void *, void **)) get_export_function(libname, "il2cpp_runtime_invoke");
...
void *Il2Cpp::GetMethodOffset(const char *image, const char *namespaze, const char *clazz, const char *name, int argsCount) {
    ...
    void *img = GetImage(image);
    if(!img) {
        IL2CPP_LOGI("找不到图片%s!", image);
        return 0;
    }

    void *klass = GetClass(image, namespaze, clazz);
    if(!klass) {
        IL2CPP_LOGI("找不到方法的类%s!", clazz, name);
    ...
    if(!method) {
        IL2CPP_LOGI("在类中找不到方法%s!", name, clazz);
        return 0;
    }
